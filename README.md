...
- Use the LAN IP address printed in the terminal when the server starts
...
- `gameServer.js` - Shared multiplayer game logic for Socket.IO
- `api/socket.js` - Vercel serverless entry point for Socket.IO
...
- Check if port 33337 (or the value of the `PORT` environment variable) is available
...
2. Note the network IP address printed in the terminal (e.g., http://192.168.1.187:33337)
...
This project ships with a serverless Socket.IO endpoint (`api/socket.js`) so it can run entirely on Vercel:

1. Install the [Vercel GitHub App](https://vercel.com/docs/concepts/git/vercel-for-github) or connect the repository manually in the Vercel dashboard.
2. When prompted for the project framework choose **Other** so Vercel serves the static HTML files.
3. No environment variables are required for the multiplayer server, but you can override the defaults with:
   - `PORT` – optional, only used when running locally. Defaults to `33337`.
   - `SOCKET_PATH` – optional Socket.IO path override. Defaults to `/api/socket` on Vercel and `/socket.io` locally.
4. Ensure the `api/socket.js` serverless function is enabled (default for Vercel projects that contain an `api/` directory).

### GitHub Actions & Vercel CI/CD

The repository includes a workflow at `.github/workflows/ci.yml` that installs dependencies for every push/PR and triggers Vercel deployments:

- **Preview deployments** run for every pull request (`prod: false`).
- **Production deployments** run for pushes to the `main` branch (`prod: true`).

Add the following GitHub secrets before enabling the workflow:

| Secret | Description |
| ------ | ----------- |
| `VERCEL_TOKEN` | Personal token from the Vercel dashboard. |
| `VERCEL_ORG_ID` | Team/organization ID for the Vercel project. |
| `VERCEL_PROJECT_ID` | Project ID generated by Vercel when you link the repository. |

The workflow uses [`amondnet/vercel-action`](https://github.com/amondnet/vercel-action) to deploy, so no additional setup is required once the secrets are present.
